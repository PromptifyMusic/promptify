...
# UWAGA: Upewnij się, że nazwa pliku jest poprawna!
...
def update_vectors():
    ...
    try:
        # 1. Włącz rozszerzenie vector i dodaj kolumnę do tabeli TAGÓW
        print("Aktualizacja schematu bazy (tabela tags_unique)...")
        with engine.connect() as conn:
            conn.execute(text("CREATE EXTENSION IF NOT EXISTS vector"))
            conn.execute(text("ALTER TABLE tags_unique ADD COLUMN IF NOT EXISTS tag_embedding vector(768)"))
            conn.commit()

        # 2. Wczytaj Parquet z TAGAMI
        print(f"Wczytywanie wektorów z: {PARQUET_PATH}")
        if not os.path.exists(PARQUET_PATH):
            print(f"BŁĄD: Nie znaleziono pliku {PARQUET_PATH}")
            return

        df = pd.read_parquet(PARQUET_PATH)
        print(f"Kolumny w pliku: {df.columns.tolist()}")

        # Wykrywanie nazw kolumn (dostosuj jeśli masz inne w pliku parquet)
        ...
        if col_name_vec not in df.columns:
            print(f"BŁĄD: Nie znaleziono kolumny z wektorem (szukano: {col_name_vec})")
            return

        # Mapowanie w pamięci: { "rock": [0.1, 0.2...], "pop": [...] }
        print("Indeksowanie danych w pamięci...")
        vector_map = {}
        ...
        session.commit()
        print(f"SUKCES! Zaktualizowano {updated_count} z {len(tags_db)} tagów.")
